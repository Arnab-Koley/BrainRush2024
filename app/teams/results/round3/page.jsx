...
const Round3 = () => {
  ...
  const fetchTeams = async () => {
    try {
      let apiUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/api/team/rounds?&page=${pageNum}&selected=${notSelected}&filter=third`;
  ...
  return (
    <section className="allteam">
      {loading ? (
        ...
        <section className="allteam">
          <div className="head">Round 3 Result</div>
...
//         try {
//             const { data } = await axios.get(
//                 `${process.env.NEXT_PUBLIC_BASE_URL}/api/team/rounds?search=${search}&page=${pageNum}&filter=third`
//             );
